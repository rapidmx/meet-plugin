...
describe("Mongo model default construction", () => {
    it("VideoMeetingMongo falls back to class defaults when constructed with no data.", () => {
        const obj = new VideoMeetingMongo();
    ...
    it("VideoMeetingMongo applies provided overrides when constructed with data.", () => {
    ...
    it("VideoMeetingMongo keeps class defaults for fields omitted from a partial constructor call.", () => {
        const obj = new VideoMeetingMongo({ mailboxUid: "mailbox-1", title: "Standup" });
    ...
    it("VideoMeetingMongo honors an explicit clear of a nullable field.", () => {
        const cleared = new VideoMeetingMongo({ calendarEventUid: null as any, publicSlug: undefined });
    ...
    it("VideoMeetingInviteeMongo falls back to class defaults when constructed with no data.", () => {
        const obj = new VideoMeetingInviteeMongo();
    ...
    it("VideoMeetingInviteeMongo applies provided overrides when constructed with data.", () => {
        const obj = new VideoMeetingInviteeMongo({
    ...
    it("VideoMeetingInviteeMongo keeps class defaults for fields omitted from a partial constructor call, and honors an explicit clear.", () => {
        const partial = new VideoMeetingInviteeMongo({ meetingUid: "meeting-1", email: "grace@example.com" });
        expect(partial.meetingUid).toBe("meeting-1");
        expect(partial.displayName).toBeUndefined();

        const empty = new VideoMeetingInviteeMongo({});
        expect(empty.meetingUid).toBe("");
        expect(empty.email).toBe("");

        const cleared = new VideoMeetingInviteeMongo({ displayName: undefined });
        expect(cleared.displayName).toBeUndefined();
    });

    it("marks both models as mailbox-scoped data, so a mailbox's erasure removes them.", () => {
        expect(isMailboxScopedData(VideoMeetingMongo)).toBe(true);
        expect(isMailboxScopedData(VideoMeetingInviteeMongo)).toBe(true);
    });

    it("declares VideoMeeting's per-record ACL (recordACL: true) and VideoMeetingInvitee's deny-all class-only ACL.", () => {
        expect(Reflect.getMetadata("rrst:recordACL", VideoMeetingMongo)).toBe(true);
        expect(Reflect.getMetadata("rrst:classACL", VideoMeetingMongo)).toEqual({
            uid: "VideoMeeting",
            records: [
        ...
        expect(Reflect.getMetadata("rrst:recordACL", VideoMeetingInviteeMongo)).toBe(false);
        expect(Reflect.getMetadata("rrst:classACL", VideoMeetingInviteeMongo)).toEqual({
